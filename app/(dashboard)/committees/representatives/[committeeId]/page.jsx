...
const EditCommitteeRepresentativesPage = () => {
  ...
  useEffect(() => {
    ...
    const fetchCommitteeDetails = async () => {
        ...
        try {
            ...
            form.setFieldsValue({
                name: committee.name,
                cards: committee.cards.map(card => ({
                    title: card.title,
            ...
            const initialMembers = committee.cards.flatMap(card => card.positions).map(p => ({id: p.memberId, name: p.memberName}));
            setMembers(initialMembers);
        } catch (error) {
            console.error("Failed to fetch committee details", error);
            message.error("Failed to fetch committee details");
        } finally {
            setLoading(false);
        }
    }
  ...
  const onFinish = async (values) => {
    ...
    try {
      ...
      message.error("Failed to update committee");
    }
  };
    ...
    >
      ...
      >
        ...
        >
          ...
          >
            Edit Committee Representatives: {committeeName}
          </Title>
          <Text type="secondary" style={{ fontSize: "14px" }}>
            Update the organizational structure and assigned members
          </Text>
        </div>
...
const EditCommitteeRepresentativesPageWithApp = () => (
  <App>
    <EditCommitteeRepresentativesPage />
  </App>
);

export default EditCommitteeRepresentativesPageWithApp;
