...
export const searchFamiliesWithCount = async (searchData) => {
  try {
    const response = await api.post(`/families/searchFamilyMembersWithCount`, searchData);
    return response.data;
  } catch (error) {
    console.error("Error searching families with count:", error);
    throw error;
  }
};
